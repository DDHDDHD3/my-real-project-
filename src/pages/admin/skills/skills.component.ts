  ...
  template: `
    <div class="space-y-8">
      <div>
        ...
        <button *ngIf="auth.canEdit()" (click)="saveSkills()" 
            ...
            <div class="flex justify-between items-start border-b border-gray-50 pb-6">
              <div class="space-y-2 group/title flex-1">
                ...
                <div class="flex items-center gap-3">
                  ...
                  <button *ngIf="auth.canEdit()" (click)="editCategoryTitle(catIndex)" class="opacity-100 md:opacity-0 md:group-hover/title:opacity-100 p-2 text-gray-400 hover:text-blue-500 transition-all">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                  </button>
                </div>
              </div>
              <button *ngIf="auth.canEdit()" (click)="removeCategory(catIndex)" class="p-3 bg-red-50 hover:bg-red-100 text-red-400 hover:text-red-600 rounded-2xl transition-all shadow-sm" title="Remove Category">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-6">
              @for (skill of category.skills; track skillIndex; let skillIndex = $index) {
                <div class="p-6 rounded-[2rem] bg-gray-50/50 border border-gray-100 flex items-center justify-between group hover:bg-white hover:shadow-2xl hover:shadow-blue-500/10 transition-all min-h-[100px] gap-4">
                  ...
                  <div *ngIf="auth.canEdit()" class="flex items-center gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all">
              ...
              <button *ngIf="auth.canEdit()" (click)="addSkillTo(catIndex)" 
      ...
      <div class="flex justify-center pt-8" *ngIf="auth.canEdit()">
